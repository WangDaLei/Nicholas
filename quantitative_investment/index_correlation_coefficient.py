...
def get_correlation_coefficient(name='上证指数', start_date='2010-01-01', end_date=date.today()):
    ...
    """
    records = IndexRecord.objects.filter(
        name=name, date__range=[start_date, end_date]).order_by('date')
    trade_amount_list = []
    change_percent_list = []
    for i, record in enumerate(records):
        if not i:
            ...
            rate = abs(round((record.close_index - records[i - 1].close_index) / records[i - 1]
                       .close_index * 100, 2))
            change_percent_list.append(rate)
    rate = get_covariance(trade_amount_list, change_percent_list) /\
        (get_standard_deviation(trade_amount_list) *
...
def get_correlation_coefficient_of_index(start_date='2010-01-01', end_date=date.today()):
    ...
    Caculate.
        ...
        计算 两个指数之间的相关系数
    """
    shang_records = IndexRecord.objects.filter(
        name='上证指数', date__range=[start_date, end_date]).order_by('date')
    shen_records = IndexRecord.objects.filter(
        name='深证指数', date__range=[start_date, end_date]).order_by('date')
    ...
    rate = get_covariance(shang_list, shen_list) /\
        (get_standard_deviation(shang_list) *
...
def get_correlation_coefficient_of_index_percent(start_date='2010-01-01', end_date=date.today()):
    ...
    for i, record in enumerate(shang_records):
        if not i:
            ...
            rate = round((record.close_index - shang_records[i - 1].close_index) /
                         shang_records[i - 1].close_index * 100, 2)
    ...
    for i, record in enumerate(shen_records):
        if not i:
            ...
            rate = round((record.close_index - shen_records[i - 1].close_index) /
                         shen_records[i - 1].close_index * 100, 2)
